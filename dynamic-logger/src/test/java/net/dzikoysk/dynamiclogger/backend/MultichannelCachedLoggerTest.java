...
final class MultichannelCachedLoggerTest {

    @Test
    void shouldStoreMessages() {
        MultichannelCachedLogger logger = new MultichannelCachedLogger(3);
        ...
        assertEquals(3, logger.getMessages().get(Channel.INFO).size());
    }

    @Test
    void shouldFindMessage() {
        MultichannelCachedLogger logger = new MultichannelCachedLogger(2);
    ...
    void shouldGetMessages() {
        ...
        testList.add("INFO First message");
        testList.add("INFO Second message");
        testList.add("INFO Third message");

        assertNotEquals(logger.getMessages().get(Channel.INFO), testList);

        testList.remove(0);

        assertEquals(logger.getMessages().get(Channel.INFO), testList);
    }
